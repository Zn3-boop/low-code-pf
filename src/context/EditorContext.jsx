...
import { normalizeProps } from '../utils/propNormalizer';
...
export const EditorProvider = ({ children }) => {
  ...
  const updateComponentProps = useCallback((componentId, newProps) => {
    // 标准化新属性
    const normalizedNewProps = normalizeProps(newProps, 'component');

    const updateNode = (node) => {
      if (node.id === componentId) {
        return { ...node, props: { ...node.props, ...normalizedNewProps } };
      }
      if (node.children) {
        return {
          ...node,
  ...
  const selectPage = useCallback((pageId) => {
    ...
    // 恢复新页面的选中组件ID
    const savedSelectedId = pageSelectedIds[pageId];
    setSelectedId(savedSelectedId || null);
  }, [activePageId, selectedId, pageSelectedIds]);
